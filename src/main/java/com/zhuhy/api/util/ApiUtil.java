...
public class ApiUtil {
	...
	public static String concatSignString(HttpServletRequest request) {
		...
		request.getParameterMap().forEach(
				(key, value) -> paramterMap.put(key, value[0]));
		// 按照key升续排序，然后拼接参数
		...
		for (String k : keyArray) {
			...
			if (paramterMap.get(k).trim().length() > 0) {
				// 参数值为空，则不参与签名
				sb.append(k).append("=").append(paramterMap.get(k).trim())
						.append("&");
			}
		}
	...
	public static NotRepeatSubmit getNotRepeatSubmit(Object handler) {
		if (handler instanceof HandlerMethod) {
			...
			NotRepeatSubmit annotation = method
					.getAnnotation(NotRepeatSubmit.class);
